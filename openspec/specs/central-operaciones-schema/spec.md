...
Fase 5 (final of EPIC-15) makes the queue multi-device-safe and client-communicating: realtime `postgres_changes` on `botellones` with a scroll/reorder gate and floating chip that never reorders under the finger (REQ-COS-27), the WhatsApp bottom sheet with the locked per-estado `mensajeWhatsApp` literal and `wa.me` deep link (REQ-COS-28), the client ficha bottom sheet with the all-estados bottle list (REQ-COS-29), and the fase-5 test contract (REQ-COS-30). REQ-COS-17 gains live realtime tab counters; REQ-COS-18 and REQ-COS-23 wire the previously-inert WhatsApp and name targets.
