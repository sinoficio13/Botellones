...
export function LabelPreview({ codigo, logoUrl, negocio, eslogan, cta, telefono }: Props) {
      ...
      <div className="grid">
        <div className="label">
          <div className="label-top">
            <h1 className="label-name">{negocio}</h1>
            {eslogan && <p className="label-slogan">{eslogan}</p>}
          </div>

          <div className="label-qr">
            <QrCodeInline codigo={codigo} size={sizePx} logoUrl={logoUrl} />
            {cta && <p className="label-cta">{cta}</p>}
          </div>

          {telefono && (
            <div className="label-whatsapp">
              <WhatsAppIcon />
              <span>{telefono}</span>
            </div>
          )}

          <p className="label-code">{codigo}</p>
        </div>
      </div>
    </>
  );
