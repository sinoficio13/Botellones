...
> Part of change `central-op-fase1-schema` — FIFO state-age foundation for the Central de Operaciones: `estado_desde` + backfill, `movimientos` audit + trigger, `mover_botellones` transactional batch RPC (SQL machine mirror + role guard), `GrupoCliente`/`agrupar()` grouping util, DB type updates. No UI. Old writers (`moverBotellon`/`updateBotellon`) stay untouched; the trigger retrofits them.
